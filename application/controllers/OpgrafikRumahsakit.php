...
class OpgrafikRumahsakit extends CI_Controller {
	...
	public function index(){
        ...
        $this->load->view('operator/VO_grafikRumahsakit',$data); // Tampilkan tampilan Gambar_v3 dengan data filter
    ...
    public function generatePdf() {
        ...
        // Siapkan data untuk grafik seperti pada method generateChart
        $borData = [];
        $avlosData = [];
        $toiData = [];
        $btoData = [];
        ...
        // Load view PDF untuk menampilkan grafik dalam PDF
        $view_data['graphData'] = json_encode($graphData);
        $view_data['first_bulan'] = $first_bulan;
        $view_data['second_bulan'] = $second_bulan;
        $pdf_content = $this->load->view('grafik/Pdf_GrafikbarRumahsakit', $view_data, true);
        
        // Generate PDF
        $pdf = new Pdf('L', 'mm', 'A4', true, 'UTF-8', false);
        $pdf->SetCreator(PDF_CREATOR);
        $pdf->SetTitle('Grafik Rumah Sakit');
        $pdf->SetHeaderData('', 0, 'Grafik Rumah Sakit', '');
        $pdf->SetMargins(15, 15, 15);
        $pdf->AddPage();
        $pdf->writeHTML($pdf_content, true, false, true, false, '');
    
        // Output PDF untuk ditampilkan atau diunduh
        $pdf->Output('grafik_rumah_sakit.pdf', 'I');
    }
