...
class Vpasienmasuk extends CI_Controller {
	...
	public function index(){       
       ...
       ORDER BY pasienmasuk.tgl_masuk ASC", array($first_tanggal, $second_tanggal))->result();
		...
		$this->load->view('visitor/V_pasienmasuk',$data);
        ...
        $data['pulang'] = $this->db->query("SELECT pasienmasuk.*
        FROM pasienmasuk
        WHERE pasienmasuk.tgl_masuk BETWEEN ? AND ?   
        ORDER BY pasienmasuk.tgl_masuk ASC", array($first_tanggal, $second_tanggal))->result();
